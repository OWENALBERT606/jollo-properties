...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.valuedAmount || body.taxRate) {
      const current = await db.valuation.findUnique({ where: { id: (await params).id } });
    ...
    const item = await db.valuation.update({ where: { id: (await params).id }, data: updateData });
    return NextResponse.json(item);
  } catch (err: any) {
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const payments = await db.taxPayment.count({ where: { valuationId: (await params).id } });
    if (payments > 0) return NextResponse.json({ error: "Cannot delete: payments linked to this valuation" }, { status: 400 });

    await db.valuation.delete({ where: { id: (await params).id } });
