        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          {recentLogs.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentLogs.map((log) => (
                <div key={log.id} className="flex items-start justify-between gap-3 py-2 border-b border-gray-50 last:border-0">
                  <div className="flex items-start gap-2">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${actionBadge[log.action] || "bg-gray-100 text-gray-600"}`}>
                      {log.action}
                    </span>
                    <div>
                      <div className="text-sm text-gray-700">{log.entityType}</div>
                      <div className="text-xs text-gray-400">{log.actor?.name ?? "System"}</div>
                    </div>
                  </div>
