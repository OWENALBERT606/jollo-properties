...
const TENURE_META: Record<string, { label: string; color: string; dot: string; desc: string }> = {
  TITLED: { label: "Titled", color: "bg-green-100 text-green-700 border-green-200", dot: "bg-green-500", desc: "Freehold title deeds — highest security" },
  MAILO: { label: "Mailo", color: "bg-blue-100 text-blue-700 border-blue-200", dot: "bg-brand-blue", desc: "Traditional Mailo land tenure system" },
  KIBANJA: { label: "Kibanja", color: "bg-amber-100 text-amber-700 border-amber-200", dot: "bg-amber-500", desc: "Customary occupancy rights" },
  LEASEHOLD: { label: "Leasehold", color: "bg-purple-100 text-purple-700 border-purple-200", dot: "bg-purple-500", desc: "Long-term lease agreements" },
  FREEHOLD: { label: "Freehold", color: "bg-teal-100 text-teal-700 border-teal-200", dot: "bg-teal-500", desc: "Absolute ownership rights" },
};
  ...
  return (
    <section className="py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <h2 className="text-3xl font-bold text-gray-900">Browse by <span className="text-brand-red">Category</span></h2>
            <p className="text-gray-500 mt-1">
              {total} active listings across Uganda — filter by tenure type or region
            </p>
          </div>
          <Button asChild variant="outline" className="border-brand-blue text-brand-blue hover:bg-brand-blue-pale shrink-0">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all border ${
                    activeTab === "ALL"
                      ? "bg-brand-red text-white border-brand-red shadow-sm scale-[1.02]"
                      : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                  }`}
                >
                  <span className="w-2 h-2 rounded-full bg-gray-400" />
                  All
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === "ALL" ? "bg-white/20" : "bg-gray-100"}`}>
                ...
                {tenureTabs.map(([tenure, props]) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all border ${
                        active
                          ? `${meta.color} border shadow-sm scale-[1.02]`
                          : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"
                      }`}
                    >
        ...
        {tabType === "tenure" && activeTab !== "ALL" && TENURE_META[activeTab] && (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex items-center gap-3 mb-5 bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm"
          >
            <span className={`w-3 h-3 rounded-full ${TENURE_META[activeTab].dot}`} />
            <span className="text-sm text-gray-600">
              <strong className="text-gray-800">{TENURE_META[activeTab].label} Land:</strong>{" "}
              {TENURE_META[activeTab].desc}
            </span>
            <span className="ml-auto text-sm font-semibold text-brand-blue">
              {currentProps.length} listings
            </span>
          </motion.div>
        )}

        {/* ── Property grid — 6 per row (2 cols mobile, 3 tablet, 6 desktop) ── */}
        <AnimatePresence mode="wait">
          <motion.div
          ...
          >
            {displayed.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                ...
                <p className="text-gray-300 text-sm mt-1">Check back soon or browse all listings</p>
              </div>
            ) : (
